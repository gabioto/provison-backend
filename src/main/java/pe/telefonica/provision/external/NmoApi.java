...
public class NmoApi {
	...
	public boolean notDoneActivity(String idActivity, String tokenExternal) throws Exception {
		RestTemplate restTemplate = new RestTemplate(initClientRestTemplate);
		restTemplate.getMessageConverters().add(new MappingJackson2HttpMessageConverter());
		...
		try {
			...
			endHour = LocalDateTime.now(ZoneOffset.of("-05:00"));
			loggerApi.thirdLogEvent("NMO", "notDoneActivity", new Gson().toJson(entity.getBody()), new Gson().toJson(responseEntity.getBody()), url,
					startHour, endHour, responseEntity.getStatusCodeValue());
