...
public class ProvisionUpdateTobeServiceImpl extends ProvisionUpdateServiceImpl implements ProvisionUpdateTobeService {
	...
	private boolean updateProvision(Provision provision, KafkaTOARequest kafkaTOARequest) throws Exception {
		...
		String xaIdSt = appointment.getId();
		...
		if (provisionStatus.equalsIgnoreCase(Status.IN_TOA.getStatusName())) {
			...
			return updateInToa(provision, kafkaTOARequest, inToaStatus);
		}
		...
		if (provisionStatus.equalsIgnoreCase(Status.WO_PRESTART.getStatusName())) {
			...
			update.set("log_status", listLog.add(statusLog));
...
//				if (nowDate.getHour() >= 0 && nowDate.getHour() <= 23) {
		...
		if (provisionStatus.equalsIgnoreCase(Status.WO_NOTDONE.getStatusName())) {
			...
			speech = notDoneStatus != null ? notDoneStatus.getGenericSpeech() : Status.WO_NOTDONE.getGenericSpeech();
			...
			String nameReplace = (provision.getCustomer().getName() != null
					&& !provision.getCustomer().getName().isEmpty()) ? provision.getCustomer().getName().split(" ")[0]
							: "Hola";
